...
function Wallet({user, withdraw, deposit}) {
    const [direction, setDirection] = useState(TRANSACTION_DIRECTION.DEPOSIT);
    const [amount, setAmount] = useState(0);

const onSubmit = (e) => {
    e.preventDefault();
    if(direction === TRANSACTION_DIRECTION.DEPOSIT) {
      deposit(amount);
    } else {
      withdraw(amount);
    }
      ...
      <div className="form-group row">
        ...
        <div className="col-sm-8">
          <input 
            ...
            id="wallet" 
            disabled 
            value={user.tokenBalances.tokenWallet}
          />
        </div>
      </div>
      <div className="form-group row">
        <label htmlFor="contract" className="col-sm-4 col-form-label">EldoraDex</label>
        <div className="col-sm-8">
          <input 
            className="form-control" 
            id="wallet" 
            disabled 
            value={user.tokenBalances.tokenEldoraDex}
          />
        </div>
      </div>
        ...
        <div className="form-group row">
          ...
          <div className="col-sm-8">
            <div id="direction" className="btn-group" role="group">
              <button 
                type="button" 
                className={`btn btn-secondary ${direction === TRANSACTION_DIRECTION.DEPOSIT ? 'active' : ''}`}
                onClick={() => setDirection(TRANSACTION_DIRECTION.DEPOSIT)}
              >Deposit</button>
              <button 
                type="button" 
                className={`btn btn-secondary ${direction === TRANSACTION_DIRECTION.WITHDRAW ? 'active' : ''}`}
                onClick={() => setDirection(TRANSACTION_DIRECTION.WITHDRAW)}
              >Withdraw</button>
            </div>
          </div>
        </div>
